...
def train(args, model, train_set, eval_set):
...
def main(args):
    ...
    if args.clip:
        ...
        config = MODEL[0].from_pretrained("t5-large")
    else:
        config = MODEL[0].from_pretrained("google/t5-efficient-tiny")

    tokenizer = MODEL[1].from_pretrained("t5-base" if args.t5base else "google/t5-efficient-tiny")

    if args.load_dir != "NONE":
        model = MODEL[2].from_pretrained(args.load_dir)
    else:
        if args.t5large:
             model = MODEL[2].from_pretrained("t5-large")
        else:
            model = MODEL[2].from_pretrained("t5-base" if args.t5base else "google/t5-efficient-tiny", config=config)
    ...
    global_step, tr_loss = train(args, model, train_set, eval_set)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--t5base", action='store_true')
    parser.add_argument("--t5large", action='store_true')
