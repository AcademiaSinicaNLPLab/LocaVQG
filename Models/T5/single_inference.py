...
def main():
    ...
    parser.add_argument("--index", type=str, default="000001")
    ...
    street_path = "/home/VIST/projects/MVQG_GS/streetview_data/Streetview_GPT_Retrieval.json"

    with open(street_path, "r") as f:
        street_dict = json.load(f)

    image_path = "/home/VIST/projects/MVQG_group3/data/"
    index = args.index

    captions = []

    street_address = street_dict[index]["street_address"]

    directions = ["north", "east", "south", "west"]

    for ind_ang in range(1,5):
        data = gr.processing_utils.encode_url_or_file_to_base64(image_path + index + "_" + str(ind_ang) + ".jpg")
        ...
        try:
            captions.append(f"On your {directions[ind_ang-1]}, {caption_res.json()['data'][0]}")
        except:
            captions.append("")
    ...
    captions.append(f"You are now driving in {street_address}")
