...
def main():
    ...
    engaging_classifier = torch.load('/home/VIST/justin/engaging_classifier/engaging_predictor_collin.pkl')
    ...
    for data_input in tqdm.tqdm(eval_dataloader):
        ...
        while class_res == 0:
            ...
            if class_res == 0:
                print(f"FILTERED QUESTION on {key}: {generations[0]}")
                filtered_questions[key].append(generations[0])
    ...
    json.dump(filtered_questions, open(os.path.join(args.result_dir, f"post_filtered_questions.json"),'w'), indent=2)
    print('Saved to post_filtered_questions.json')
